"""경로 생성 — 다익스트라 최단경로 + 예정경로 반영 소프트 페널티 (#3, 2026-07-20).

Graph의 인접/비용을 사용. 작업공간 랙 사이 엣지 페널티가 비용에 반영돼 있어,
경유 트래픽은 자연히 통로로 우회한다(출발/도착이면 어쩔 수 없이 지남).
penalized(상대 로봇의 남은 예정 경로 노드 — traffic._others)로 들어가는 엣지는
비용 2배 — 막지 않고 비싸게만 해서, 교차 압박이 높을수록 상대가 지나갈 구간을
우회하는 경로가 선택된다. 이 모듈은 집합만 받으므로 B안(예약 노드)→#3(예정경로)
전환에도 탐색 로직 변경 없음.
(A/B 비교: 시뮬 set2 −11.8%, 실기 −8.1% makespan — Gazebo/traffic_results)
"""
import heapq

PENALTY_FACTOR = 2.0        # 상대 예정경로 노드행 엣지 비용 배율
